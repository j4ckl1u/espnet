...
from st_utils import PlotLASTAttentionReport

from e2e_ts_th import LAST
from e2e_ts_th import LAST_Loss
from e2e_ts_th import LAST_Reporter
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device):
        ...
        clens =  np.array([c.shape[0] for c in cs])
        clens = torch.from_numpy(clens).to(device)
        ys_pad = pad_list([torch.from_numpy(y).long() for y in ys], self.ignore_id).to(device)
        cs_pad = pad_list([torch.from_numpy(c).long() for c in cs], self.ignore_id).to(device)

        return xs_pad, ilens, cs_pad, clens, ys_pad
...
def train(args):
    ...
    odim = int(valid_json[utts[0]]['output'][0]['shape_trg'][1])
    ...
    e2e = LAST(idim, odim, args)
    model = LAST_Loss(e2e, args.mtlalpha)
    ...
    if args.num_save_attention > 0 and args.mtlalpha != 1.0:
        ...
        trainer.extend(PlotLASTAttentionReport(
    ...
    trainer.extend(extensions.PlotReport(['main/loss', 'validation/main/loss',
                                          ...
                                          'main/loss_att_trg', 'validation/main/loss_att_trg'],
